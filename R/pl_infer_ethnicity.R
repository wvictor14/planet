...
pl_infer_ethnicity <- function(betas, threshold = 0.75) {
  ...
  if (length(pf) < length(planet::pl_ethnicity_features)) {
    warning(paste("Only", length(pf), "out of", 
  ...
  probs <- data.frame(aperm(pp / rep(psum, rep(nclass, nlambda * npred)), 
                            c(3, 1, 2)))
  ...
  function(x, ignore_labels = FALSE) {
    ...
    if (any(nas)) {
      ...
      if (sum(nas) < d[1]) {
        ...
        pclass[!nas] <- pclass2
        if (is.factor(pclass2)) pclass <- factor(pclass, levels = seq(d[2]), 
      ...
      dd <- dimnames(x)[[2]]
      if (!ignore_labels) pclass <- factor(pclass, levels = seq(d[2]), 
                                           labels = dd)
    }
    pclass
  }
