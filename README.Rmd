...
Australian samples. To save on memory, I only use 8/24 samples saved in this repo as an 
`minfi::RGChannelSet` object, which is can be [loaded from idat files](https://bioconductor.org/packages/release/bioc/vignettes/minfi/inst/doc/minfi.html#3_reading_data).
...
Here we apply `minfi::preprocessNoob()` and `wateRmelon::BMIQ()` to normalize the methylation data: 
...
and then we combine this with the 65 snp probe data (will be 59 SNPs if using EPIC).
