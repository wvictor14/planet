...
For demonstration purposes, I downloaded a [placental DNAm dataset from GEO](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE75196), which contains samples collected 
in an Australian population. To save on memory, I only use 8/24 samples, which I have saved in
this repo as a `minfi::RGChannelSet` object.
...
are available, then you can still run the algorithm but any differences resulting from the different
normalizations may impact accuracy. 
...
the classification which is determined by the highest class-specific probability. The former
 ...
 confidently classified to a single ethnicity group. If a sample fails this threshold, this can be
 often because of mixed ancestry, and the sample is given an `Ambiguous` label. The default 
 threshold is 0.75. 
...
analysis, and to use only 2/3 of the probabilities, since the third will be redundant (probabilities 
sum to 1).
